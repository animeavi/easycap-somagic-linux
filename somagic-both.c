...
static struct video_state_t vs = {.line = 0,.col = 0,.state = 0,.field =
	    0,.blank = 0
};
...
void set_vid_mode(libusb_device_handle * dev_handle)
{
	...
	libusb_fill_control_setup(async_ctl_buf,
				  LIBUSB_REQUEST_TYPE_VENDOR +
				  LIBUSB_RECIPIENT_DEVICE, 0x0000001,
				  0x0000001, 0x0000000, 2);

	libusb_fill_control_transfer(async_ctl, dev_handle, async_ctl_buf,
				     control_rx, NULL, 1000);
	...
	libusb_fill_control_transfer(async_set_intf, dev_handle,
				     async_set_intf_buf, int_set_2_rx, NULL,
				     1000);
...
void set_snd_mode(libusb_device_handle * dev_handle)
{
	...
	libusb_fill_control_transfer(async_set_intf, dev_handle,
				     async_set_intf_buf, int_set_1_rx, NULL,
				     1000);
	libusb_submit_transfer(async_set_intf);
...
void gotdata(struct libusb_transfer *tfr)
{
	...
	for (i = 0; i < num; i++) {
		unsigned char *data =
		    libusb_get_iso_packet_buffer_simple(tfr, i);
		...
		while (pos < length) {
			...
			if (data[pos] == 0xaa && data[pos + 1] == 0xaa
				...
				fprintf(stderr,
					"Unexpected block, expected [aa aa 00 00/01] found [%02x %02x %02x %02x]\n",
...
#ifdef DEBUG
	if (ret != 13) {
		fprintf(stderr, "read_reg control msg returned %d, bytes: ",
			ret);
		print_bytes(buf, ret);
		fprintf(stderr, "\n");
	}
...
void usage()
{
	...
	fprintf(stderr,
		"  -c, --cvbs                 Use CVBS (composite) input (default)\n");
	fprintf(stderr,
	...
	fprintf(stderr,
		"capture 2> /dev/null | mplayer - -vf yadif,screenshot -demuxer rawvideo -rawvideo \"pal:format=uyvy:fps=25\"\n");
	fprintf(stderr, "\n");
	...
	fprintf(stderr,
		"capture -n --luminance=2 --lum-aperture=3 2> /dev/null | mplayer - -vf yadif,screenshot -demuxer rawvideo -rawvideo \"pal:format=uyvy:fps=25\"\n");
}
...
int main(int argc, char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, "B:cC:f:H:npsS:", long_options,
				&option_index);
		if (c == -1) {
			break;
		}
	...
	if (!dev) {
		fprintf(stderr,
			"USB device %04x:%04x was not found. Has device initialization been performed?\n",
			VENDOR, PRODUCT);
		return 1;
	}
	...
	if (ret != 0) {
		fprintf(stderr,
			"set_interface_alt_setting failed with error %d\n",
			ret);
		exit(1);
	}
	...
	if (ret != 0) {
		fprintf(stderr,
			"failed to release interface before set_configuration: %d\n",
			ret);
	}
#endif
	...
	if (ret != 0) {
		fprintf(stderr,
			"claim after set_configuration failed with error %d\n",
			ret);
	}
#endif
	ret = libusb_set_interface_alt_setting(devh, 0, 0);
	...
	for (i = 0; i < NUM_ISO_TRANSFERS; i++) {
		...
		libusb_fill_iso_transfer(tfr[i], devh, 0x00000082, isobuf[i],
					 64 * 3072, 64, gotdata, NULL, 2000);
...
#ifdef DEBUG
			fprintf(stderr,
				"libusb_submit_transfer failed with error %d for transfer %d\n",
				ret, i);
#endif
			exit(1);
		}
	...
	while ( /*pending_requests > 0 */ 1) {
		...
		if (vid_free_item >= NUM_ISO_TRANSFERS - 4) {
			...
			fprintf(stderr, "v\n");
#endif
			for (i = 0; i < vid_free_item; i++)
				libusb_submit_transfer(vid_free[i]);
			vid_free_item = 0;
		}

...
void gotdata(struct libusb_transfer *tfr)
{
	...
	for (i = 0; i < num; i++) {
		write(1, libusb_get_iso_packet_buffer_simple(tfr, i),
		      tfr->iso_packet_desc[i].actual_length);
	}
...
int main()
{
	...
	ret =
	    libusb_control_transfer(devh,
				    ...
				    0x000000b, 0x0000000, buf, 0x0000008, 1000);
	...
	ret =
	    libusb_control_transfer(devh,
				    LIBUSB_REQUEST_TYPE_VENDOR +
				    LIBUSB_RECIPIENT_DEVICE, 0x0000001,
				    0x0000001, 0x0000000, buf, 0x0000002, 1000);
	...
	assert(tfr0 != NULL);
	libusb_fill_iso_transfer(tfr0, devh, 0x00000082, isobuf0, 32 * 1008,
				 32, gotdata, NULL, 2000);
	...
	assert(tfr1 != NULL);
	libusb_fill_iso_transfer(tfr1, devh, 0x00000082, isobuf1, 32 * 1008,
				 32, gotdata, NULL, 2000);
